...
def l_get_IOS_vrf(l_show_vrf_output):
    l_vrf = []
    for each_vrf in l_show_vrf_output:
        s_vrf = each_vrf[2:21].strip(' ')
        l_vrf.append(s_vrf)
    return l_vrf
...
def d_read_uni_req(req_file_name):
        ...
        for line in csv_input:
                i += 1
                #skip csv header and empty line
                if line == []:
                    continue
                if (i <= 6) and ('CBPDCN' not in line[0].strip(' ')):
                        continue
                else:
                        # skip empty line
                        if i <= 6:
                        ...
                        duplicated_port = 'no'
                        if d_uni_req[host_name] != []:
                            for each_current_port in d_uni_req[host_name]:
                                if port_id in each_current_port.keys():
                                    print "Warning!!! Duplicate port found: " + host_name + " " + port_id
                                    duplicated_port = 'yes'
                                    break
                        if duplicated_port == 'yes':
                            continue
...
def d_read_dcn_vlans(dcn_vlan_file_name = 'dcn_vlan.csv'):
    dcn_vlan_file = open(dcn_vlan_file_name, 'r')
    csv_input = csv.reader(dcn_vlan_file)
    d_new_DCN_vlanname = {}
    for each_vlan in csv_input:
        if each_vlan == []:
            continue
        vlan = int(each_vlan[0].strip(' '))
        name = each_vlan[1].strip(' ')
        d_new_DCN_vlanname[vlan] = {}
        d_new_DCN_vlanname[vlan]['vlan_name'] = name
        d_new_DCN_vlanname[vlan]['vxlan_id'] = vlan + 10000
    dcn_vlan_file.close()
    return d_new_DCN_vlanname
