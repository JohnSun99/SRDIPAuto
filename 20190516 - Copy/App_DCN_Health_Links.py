...
def check_DCN_fabric_links(switch_ID):
    print "\nchecking inter-switch links on " + switch_ID.hostname
    ...
    b_result = f_N9K_cdp_check (l_cdps, switch_ID.HWPlatform, s_site_location)
    if not b_result:
        f_print_cdp_details(switch_ID)



if __name__ == "__main__":
    d_host_ip = gen_funs.d_read_host_file()
    l_hosts = open('task_DCN_Health_Links.txt', 'r')
    
    for line in l_hosts:
        switch1 = DCNSwitch(d_host_ip[line.strip('\n')])
        check_DCN_fabric_links(switch1)
    l_hosts.close()
