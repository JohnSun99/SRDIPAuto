...
# 1 "D:/PythonStudy/DIPAuto/data/REQ88888.csv   ================== each new SR 
# 2 "D:/PythonStudy/DIPAuto/data/" + key + "-vlans.txt"  ================= every Leaf pair need current vlans by logging switches "show vxlan" and take the vlans
# 
# static source file:
...
def fun_create_Vlan_Vni(v_i_Vlan, v_i_DCN_AS):
        ...
        elif v_i_DCN_AS == 64522:
                i_Vni = 11000 + v_i_Vlan
                s_VlanName = d_BWIPnet_vlanname[s_Vlan]
        else:
                i_Vni = 14000 + v_i_Vlan
                print(s_Vlan)
...
s_REQ_full_filename = "D:/PythonStudy/DIPAuto/data/REQ88888.csv"
f_req = open(s_REQ_full_filename, 'r')

d_host_vlans = {}
for line in f_req:
        l_line = line.split(';')
        s_host = d_peer[l_line[0]].strip('\n')
        s_vlans = l_line[2].replace(' ','')
        l_vlans = fun_l_parse_vlans(s_vlans)
        #print (s_host)
        #print (s_vlans)
